...
to avoid-turtles
  set obstacles-turtles turtles in-cone vision angle-avoidance
  with [color != [color] of myself]
  if (any? obstacles-turtles)[
    let a angleFromVect vectFleeTurtles
    turn-towards a factor-separate
  ]
  ;;[flee-turtles]
end

to-report vectFleeTurtles
  let vr 0
  let obstacles-in-front obstacles-turtles in-cone 3 angle-flee
  ifelse (any? obstacles-in-front)[
    let cible min-one-of  obstacles-in-front [distance myself]
    set vr VectFromAngle (towards cible + 180 ) (1 / distance cible) 
  ][
  set vr VectFromAngle heading 1
  ]
  report vr
end
