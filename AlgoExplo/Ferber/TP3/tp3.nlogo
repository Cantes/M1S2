...
to search
  let list-nodes current
    
  while [get-patch current != mygoal] [
      
    ifelse( length list-nodes = 0)[
      show "Echec"    
    ][
    let cnode first list-nodes
    
    ask cnode [set pcolor blue]
      
    set list-nodes but-first list-nodes
    
    ifelse(cnode = mygoal)[
      show "Touvé!" 
    ][
    set list-nodes lput list-nodes generate cnode
    ]
    ]
  ]
end
  ...
  ;;mygoal in-nodes out-nodes current
  
to search-step
    
  let rep avoid-obstacles
  if ( rep = 0)[
    fd 1
  ]
end
...
;; Retourne les voisins possibles de p = voisins qui ne sont pas un obstable
to-report generate [p]

  let voisins ([neighbors4] of p)  with [pcolor = black]
  
  report [self] of voisins
end
...
;; cree une liste avec le min des distances en fusionnant les listes
...
to-report avoid-obstacles
  let obstacles patches in-cone 5 20 with [pcolor != black]
