...
to flock  ;; turtle procedure
  ...
  let vs find-nearest-neighbor
  let va align
  let vc cohere
 
 
  let vr additionvect ( multiplyScalarvect (1 / 3) vs ) additionvect ( multiplyScalarvect (1 / 3) va ) ( multiplyScalarvect (1 / 3) vc )
...
to-report find-nearest-neighbor ;; turtle procedure
  
  ;;si pas de voisin vs=(?,0)
  ;;sinon  vs=(toward(voisin)+180, 1/ distance voisin )
  
  let neighbor min-one-of flockmates [distance myself]
  
  ifelse ( neighbor != nobody) [
    report list (towards (neighbor) + 180) ( 1 - [distance myself] of neighbor)
  ][
    report list(0) (0)
  ]
...
to-report align  ;; turtle procedure
  ;;< O, 1>
  let r average-flockmate-heading
  report list (r) (1)
...
to-report cohere  ;; turtle procedure
  ...
  let neighbor min-one-of flockmates [distance myself]
  ifelse ( neighbor != nobody) [
     report list(average-heading-towards-flockmates) (1)
  ][
    report list(0) (0)
  ]
...
;;  let ve avoidance
 ;; let vs alignement
  ...
  ;; ae, as et sc = coefficient entre 0 et 1 en fonction de leurs importance, dans l'ideal : as + ae + ac = 1 ( vecteur normé )
  ;;let vr additionvect ( multiplyScalarvect ae ve ) additionvect ( multiplyScalarvect as vs ) additionvect ( multiplyScalarvect ac vc )
 ; 
end
