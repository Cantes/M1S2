...
long determinant(int i, int j, int k){

	long long x1= (long long) segments[i].abscisse;
	long long y1= (long long) segments[i].ordonnee;
	
	long long x2= (long long) segments[j].abscisse;
	long long y2= (long long) segments[j].ordonnee;

	long long x3= (long long) segments[k].abscisse;
	long long y3= (long long) segments[k].ordonnee;
	
	cout << i << "," << j << "," << k << " : " << ( (x3-x1)*(y2-y1) - (y3-y1)*(x2-x1) ) << endl;
...
bool Intersectent(int i, int j){	
	...
	if( determinant(i,i+1,j) * determinant(i,i+1,j+1) < 0 ){
		if(determinant(j,j+1,i) * determinant(j,j+1,i+1) < 0){
			return true;
		}else{
			return false;
		}
